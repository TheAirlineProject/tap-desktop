       ...
       [Versioning("cargostatistics")]
        ...
        public AirportFacility getCurrentAirportFacility(Airline airline, AirportFacility.FacilityType type)
        {
            ...
            lock (this.Facilities)
            {
                ...
                {
                    var typeLevel = tFacilities.Where(f=>f.Airline == airline && f.Facility.Type == type).Select(f=>f.Facility.TypeLevel);

                    if (typeLevel.Count() > 0)
                    {
                        return AirportFacilities.GetFacilities(type).Find(f => f.TypeLevel == typeLevel.First() - 1);
                    }
                    else
                    {
