...
using Sqlite3 = SQLitePCL.raw;
    ...
    namespace SQLite
    {
        ...
        {
            void BindParameter(Sqlite3Statement stmt, int index);
        ...
        public static class Orm
        {
            ...
            public static string SqlType(TableMapping.Column p, bool storeDateTimeAsTicks)
            {
                ...
                else if (clrType.GetInterfaces().Contains(typeof(IDatabase)))
                {
                    return "varchar(36)";
                }
                else
                {
                    var interfaces = clrType.GetType().GetInterfaces();
                    var type = clrType.GetType();
        ...
        public partial class SQLiteCommand
        {
            ...
            internal static void BindParameter(Sqlite3Statement stmt, int index, object value, bool storeDateTimeAsTicks)
            {
                ...
                else
                {
                    ...
                    else if (value.GetType().GetInterfaces().Contains(typeof(IDatabase)))
                    {
                        ((IDatabase)value).BindParameter(stmt, index);
                    }
                    else
                    {
            ...
            object ReadCol(Sqlite3Statement stmt, int index, SQLite3.ColType type, Type clrType)
            {
                ...
                else
                {
                    ...
                    else if (clrType.GetInterfaces().Contains(typeof(IDatabase)))
                    {
                        ...
                        var objs = new Object[] { text };
                        var oo = clrType.GetMethod("GetElement").Invoke(null, objs);
                        return oo;//SQLite3.ColumnString(stmt, index);
                    }
                    else
                    {
